...
bool Player::LoadFromDB(ObjectGuid playerGuid, CharacterDatabaseQueryHolder const& holder)
{
    ...
    // 70          71               72            73                     74       75
    //"actionBars, grantableLevels, innTriggerId, extraBonusTalentCount, hardcore, UNIX_TIMESTAMP(creation_date) FROM characters WHERE guid = '{}'", guid);
    ...
    SetHardcore(fields[74].Get<uint8>() != 0);
