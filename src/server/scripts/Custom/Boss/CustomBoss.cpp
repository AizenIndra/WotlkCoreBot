                ...
                for (WorldSessionMgr::SessionMap::const_iterator it = sessions.begin(); it != sessions.end(); ++it)
                {
                    if (Player* player = it->second->GetPlayer())
                    {
                        if (player->IsInWorld() && player->GetAreaId() == areaId && player->GetTeamId() == (alliance ? TEAM_ALLIANCE : TEAM_HORDE)) {
                            ...
                            char const* winEventBossMsg =
                                (player->GetSession()->GetSessionDbLocaleIndex() == LOCALE_ruRU)
                                    ? "Ваша фракция победила босса события! Вы получили награду."
                                    : "Your faction defeated the event boss! You received a reward.";
                            ChatHandler(player->GetSession()).PSendSysMessage("%s", winEventBossMsg);
                        }
                    }
                }
            }
